    ...
    <section className="py-80 bg-light" id="blog">
      <div className="container">
        <div className="section-title text-center">
          ...
          <p className="section-desc">Stay updated with crop updates, quality benchmarks, and product sourcing guides from Jaliyan Exim experts.</p>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
          {posts.map((post, idx) => (
            <div key={idx} style={{ backgroundColor: '#fff', borderRadius: '16px', border: '1.5px solid var(--border)', overflow: 'hidden', boxShadow: '0 8px 24px rgba(14, 165, 233, 0.05)', display: 'flex', flexDirection: 'column' }}>
              <div style={{ position: 'relative', height: '200px', backgroundColor: '#FFFFFF', padding: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <span style={{ position: 'absolute', bottom: 12, right: 12, background: 'linear-gradient(135deg, #0EA5E9 0%, #38BDF8 100%)', color: '#FFFFFF', fontSize: '11px', fontWeight: 800, padding: '4px 10px', borderRadius: '100px', boxShadow: '0 2px 8px rgba(56, 189, 248, 0.3)' }}>
                  {post.date}
                </span>
              </div>
