// Centralized Dynamic Data & Admin Store — Jaliyan Exim
...
import { DEHYDRATED_PRODUCTS as INITIAL_DEHYDRATED_PRODUCTS, DEHYDRATED_CATEGORIES } from '../data/dehydratedProducts';
...
function getInitialList(lsKey, fallback) {
  ...
  try {
    ...
    if (raw) {
      ...
      if (lsKey === 'trishu_products' && Array.isArray(parsed)) {
        return parsed.filter(item => {
          const id = (item.id || '').toLowerCase();
          const cat = (item.category || item.cat || '').toLowerCase();
          return !id.startsWith('dehydrated-') && !cat.includes('dehydrated');
        });
      }
      return parsed;
    }
...
// Resilient Image Normalizer — ALWAYS preserves custom user-uploaded/edited images & incorporates newly added products
const INITIAL_DEHYDRATED_MAP = new Map(INITIAL_DEHYDRATED_PRODUCTS.map(p => [p.id, p]));

function sanitizeDehydratedList(list) {
  if (!Array.isArray(list) || list.length === 0) return INITIAL_DEHYDRATED_PRODUCTS;
  
  const sanitized = list.map(item => {
    const defaultItem = INITIAL_DEHYDRATED_MAP.get(item.id);
    return {
      ...item,
      image: (defaultItem && defaultItem.image) ? defaultItem.image : item.image,
      category: (defaultItem && defaultItem.category) ? defaultItem.category : item.category,
      cat: (defaultItem && defaultItem.cat) ? defaultItem.cat : item.cat,
      categories: (defaultItem && defaultItem.categories) ? defaultItem.categories : item.categories,
      desc: (defaultItem && defaultItem.desc) ? defaultItem.desc : item.desc
    };
  });

  const existingIds = new Set(sanitized.map(p => p.id));
  INITIAL_DEHYDRATED_PRODUCTS.forEach(p => {
...
function sanitizeProductList(list) {
  ...
  // Strictly filter out any dehydrated products so Spices catalog is 100% pure spices
  const nonDehydrated = list.filter(item => {
    const id = (item.id || '').toLowerCase();
    const cat = (item.category || item.cat || '').toLowerCase();
    return !id.startsWith('dehydrated-') && !cat.includes('dehydrated');
  });

  const sanitized = nonDehydrated.map(item => {
...
export const DEFAULT_MAIN_CATEGORIES = [
  { id: 'spices', name: 'Spices & Seasonings', icon: 'Sparkles', color: '#0EA5E9', defaultHs: 'HS 0910', defaultPack: '25kg / 50kg PP Bags' },
  { id: 'agro', name: 'Agro Commodities', icon: 'Sprout', color: '#138808', defaultHs: 'HS 1006', defaultPack: '25kg / 50kg PP Bags / Bulk FCL' },
  { id: 'dehydrated', name: 'Dehydrated Fruits & Veg', icon: 'Leaf', color: '#0284C7', defaultHs: 'HS 0712', defaultPack: '14kg / 20kg Poly Bags in Cartons' }
];

function getInitialMainCategories() {
  if (typeof window === 'undefined') return DEFAULT_MAIN_CATEGORIES;
  try {
    // Clear old category data that had extra categories
...
export const INITIAL_CATEGORIES = {
  ...
  dehydrated: DEHYDRATED_CATEGORIES,
  ...
  try {
    ...
    if (raw) {
      ...
      if (parsed && typeof parsed === 'object' && Object.keys(parsed).length > 0) {
        if (Array.isArray(parsed.spices)) {
...
const STORE_KEYS = [
  ...
  { idbKey: 'dehydrated', memKey: 'dehydrated', lsKey: 'jaliyan_dehydrated_products' },
...
async function initIndexedDBStore() {
  ...
  for (const k of STORE_KEYS) {
    ...
    if (fromIdb && Array.isArray(fromIdb) && fromIdb.length > 0) {
      if (k.memKey === 'products') {
        const mergedProds = sanitizeProductList(fromIdb);
        memoryCache[k.memKey] = mergedProds;
        await idbSet(k.idbKey, mergedProds);
      } else if (k.memKey === 'mainCategories') {
        const mainCatIds = new Set(fromIdb.map(c => c.id));
        const mergedMain = [...fromIdb];
        DEFAULT_MAIN_CATEGORIES.forEach(dc => {
          if (!mainCatIds.has(dc.id)) mergedMain.push(dc);
        });
        memoryCache[k.memKey] = mergedMain;
        await idbSet(k.idbKey, mergedMain);
      } else {
  ...
  // Load custom categories from IndexedDB and ensure fresh categories exist
  ...
  if (categoriesFromIdb && typeof categoriesFromIdb === 'object' && Object.keys(categoriesFromIdb).length > 0) {
    const mergedCats = { ...INITIAL_CATEGORIES, ...categoriesFromIdb };
    // Ensure spices, agro, dehydrated have all default categories
    ['spices', 'agro', 'dehydrated'].forEach(dom => {
      const defCats = INITIAL_CATEGORIES[dom] || [];
      const currCats = mergedCats[dom] || [];
      const setCats = new Set(currCats);
      defCats.forEach(c => {
        if (!setCats.has(c)) currCats.push(c);
      });
      mergedCats[dom] = currCats;
    });
    memoryCache.categories = mergedCats;
    await idbSet('categories', mergedCats);
    hasUpdates = true;
  } else {
...
// --- AUTHENTICATION ---
const AUTH_STORAGE_KEY = 'jaliyan_admin_auth_v1';
...
export function loginAdmin(username, password) {
  ...
  if (
    (cleanUser === 'jaliyan exim' || cleanUser === 'jaliyanexim') &&
    cleanPass === 'jaliyanexim@123'
  ) {
    sessionStorage.setItem(AUTH_STORAGE_KEY, 'true');
    return { success: true };
  }
  return { success: false, message: 'Invalid Admin ID or Password. (ID: jaliyan exim)' };
}
...
export function getProducts() {
  return sanitizeProductList(memoryCache.products).filter(item => {
    const id = (item.id || '').toLowerCase();
    const cat = (item.category || item.cat || '').toLowerCase();
    return !id.startsWith('dehydrated-') && !cat.includes('dehydrated');
  });
...
// --- DEHYDRATED FRUITS & VEGETABLES STORE ---
export function getDehydratedProducts() {
  return sanitizeDehydratedList(memoryCache.dehydrated || []);
}

export function saveDehydratedProducts(list) {
  persistData('dehydrated', 'dehydrated', 'jaliyan_dehydrated_products', list);
}

export function addDehydratedProduct(newProd) {
  const list = getDehydratedProducts();
  const prodWithId = {
    ...newProd,
    id: newProd.id || `dehydrated-${Date.now()}`
  };
  const updated = [prodWithId, ...list];
  saveDehydratedProducts(updated);
  setCloudSingleItem('dehydrated', prodWithId).catch(() => {});
  return updated;
}

export function updateDehydratedProduct(updatedProd) {
  const list = getDehydratedProducts();
  const updated = list.map(p => (String(p.id) === String(updatedProd.id) ? { ...p, ...updatedProd } : p));
  saveDehydratedProducts(updated);
  setCloudSingleItem('dehydrated', updatedProd).catch(() => {});
  return updated;
}

export function deleteDehydratedProduct(id) {
  const list = getDehydratedProducts();
  const updated = list.filter(p => String(p.id) !== String(id));
  saveDehydratedProducts(updated);
  deleteCloudSingleItem('dehydrated', id, updated).catch(() => {});
  return updated;
}
...
const DEFAULT_EMAIL_CONFIG = {
  recipientEmail: 'jaliyanexim2706@gmail.com',
  enabled: true,
  web3FormsKey: ''
};
...
export async function sendEnquiryEmail(enquiryData) {
  ...
  const recipient = (config.recipientEmail || 'jaliyanexim2706@gmail.com').trim();
  if (!recipient) return { success: false, message: 'No recipient email configured' };

  const subject = `🚨 New Enquiry from ${enquiryData.name || 'Website Visitor'} (${enquiryData.product || enquiryData.source || 'General'}) - Jaliyan Exim`;
    ...
    try {
      const resp = await fetch('https://api.web3forms.com/submit', {
        ...
        body: JSON.stringify({
          ...
          subject: subject,
          from_name: 'Jaliyan Exim Website',
...
export function exportEnquiriesCSV(filter = 'all') {
  ...
  link.setAttribute('download', `jaliyan_enquiries_export_${new Date().toISOString().split('T')[0]}.csv`);
...
function normalizeDomainKey(domain) {
  ...
  if (d === 'dehydrated' || d === 'dehydrated-products' || d === 'dehydrated-fruits-vegetables') return 'dehydrated';
  return d;
}

export function getCategories(domain) {
  const normKey = normalizeDomainKey(domain);
  let list = (memoryCache.categories && memoryCache.categories[normKey]) || INITIAL_CATEGORIES[normKey] || ['All'];
  if (normKey === 'spices') {
    list = list.filter(c => !c.toLowerCase().includes('dehydrated'));
  }
...
export function updateCategory(domain, oldName, newName) {
  ...
  } else if (normKey === 'dehydrated') {
    const prods = getDehydratedProducts().map(p => (p.category === oldTrimmed || p.cat === oldTrimmed) ? { ...p, category: newTrimmed, cat: newTrimmed } : p);
    saveDehydratedProducts(prods);
...
export function deleteCategory(domain, categoryName) {
  ...
  if (normKey === 'spices') {
    ...
    const prods = getDehydratedProducts().map(p => (p.category === trimmed || p.cat === trimmed) ? { ...p, category: fallbackCat, cat: fallbackCat } : p);
    saveDehydratedProducts(prods);
  } else if (normKey === 'sanitaryware') {
...
export function getDomainProducts(domainId) {
  ...
  if (normKey === 'dehydrated') return getDehydratedProducts();
...
export function addDomainProduct(domainId, product) {
  ...
  if (normKey === 'dehydrated') return addDehydratedProduct(product);
...
export function updateDomainProduct(domainId, product) {
  ...
  if (normKey === 'dehydrated') return updateDehydratedProduct(product);
...
export function deleteDomainProduct(domainId, productId) {
  ...
  if (normKey === 'dehydrated') return deleteDehydratedProduct(productId);
