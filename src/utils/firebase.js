...
// New site config key — alag localStorage key use hogi Jaliyan Exim se
const CONFIG_STORAGE_KEY = 'new_site_firebase_config';

// Firebase DISCONNECTED — Jaliyan Exim se alag nayi site ke liye
// Naya Firebase project connect karne ke liye Admin Panel > Cloud Sync use karo
...
export async function getCloudData(collectionKey) {
  ...
  try {
    ...
    const colName = `jaliyan_items_${collectionKey}`;
    ...
    const docRef = doc(db, 'jaliyan_store', collectionKey);
...
export async function deleteCloudSingleItem(collectionKey, itemId, remainingList = null) {
  ...
  try {
    ...
    if (Array.isArray(remainingList)) {
      try {
        const mainDocRef = doc(db, 'jaliyan_store', collectionKey);
